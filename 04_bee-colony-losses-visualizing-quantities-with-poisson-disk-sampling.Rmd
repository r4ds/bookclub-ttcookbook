...
colonyplot2 |> 
  ...
  facet_wrap(~year) +
  labs(title="2015-20 Fall Bee Counts") +
  theme_statebins(base_family = "red hat") +
  theme(plot.title=element_text(size=16, hjust=0)) +
  theme(plot.margin = margin(30,30,30,30)) +
  theme(
    legend.key.size = unit(1.5, "cm"), # Increase the key box size
    legend.title = element_text(size = 6, face = "bold"), # Increase title size and make bold
    legend.text = element_text(size = 6) # Increase label size
  ) +
  guides(
    color = guide_legend(override.aes = list(size = 3)), # Increase symbol size for 'color' legend
    shape = guide_legend(override.aes = list(size = 3))  # Increase symbol size for 'shape' legend
  ) +
